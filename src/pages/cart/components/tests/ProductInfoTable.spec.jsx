...
it('장바구니에 포함된 아이템들의 이름, 수량, 합계가 제대로 노출된다', async () => {
  ...
  expect(within(firstItem).getByText('$2,427.00')).toBeInTheDocument(); // 가격 * 수량

  expect(
    within(secondItem).getByText('Awesome Concrete Shirt'),
  ).toBeInTheDocument();
  expect(within(secondItem).getByRole('textbox')).toHaveValue('4');
  expect(within(secondItem).getByText('$1,768.00')).toBeInTheDocument(); // 가격 * 수량
});
...
it('특정 아이템의 수량이 1000개로 변경될 경우 "최대 999개 까지 가능합니다!"라고 경고 문구가 노출된다', async () => {
  ...
  // jsdom window 객체 전역에 alert 함수를 스파이 함수로 대체
...
it('특정 아이템의 삭제 버튼을 클릭할 경우 해당 아이템이 사라진다', async () => {
  ...
  // queryByText는 해당 요소가 없을 때 에러가 발생하지 않기 때문에 없는 것을 확인하고자 할 때는 queryByText를 사용하는 것이 좋음
